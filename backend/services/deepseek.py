...
class DeepSeekService:
    """Async client for the DeepSeek (OpenAI-compatible) API."""
    ...
    async def ask(self, character: Character, user_message: str) -> str:
        """
        Send a message to the DeepSeek API and return the assistant reply text.

        Raises:
            DeepSeekAPIError: on non-2xx responses or unexpected payload shape.
        ...
        if response.status_code != 200:
            ...
            raise DeepSeekAPIError(response.status_code, detail)
        ...
        try:
            ...
            raise DeepSeekAPIError(
                500, f"Unexpected response shape from DeepSeek API: {exc}"
            ) from exc
